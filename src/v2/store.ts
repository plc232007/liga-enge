...
export class Store {
 ...
 `);
    ...
    all<T>(collection: Collection): T[] { return (this.db.prepare('SELECT data FROM v2_entity WHERE collection=? ORDER BY rowid').all(collection) as {
        data: string;
    }[]).map(r => JSON.parse(r.data)); }
    get<T>(collection: Collection, id: string): T | undefined { const r = this.db.prepare('SELECT data FROM v2_entity WHERE collection=? AND id=?').get(collection, id) as {
        data: string;
    } | undefined; return r ? JSON.parse(r.data) : undefined; }
    put<T extends {
        id: string;
    }>(collection: Collection, item: T) { this.db.prepare('INSERT INTO v2_entity(collection,id,data) VALUES (?,?,?) ON CONFLICT(collection,id) DO UPDATE SET data=excluded.data').run(collection, item.id, JSON.stringify(item)); return item; }
    audit(entity: string, entityId: string, action: string, before: unknown, after: unknown, authorId: string) { const item: Audit = { id: randomUUID(), entity, entityId, action, before, after, authorId, createdAt: new Date().toISOString() }; this.db.prepare('INSERT INTO v2_audit VALUES (?,?)').run(item.id, JSON.stringify(item)); }
    transaction<T>(fn: () => T): T { return this.db.transaction(fn)(); }
    points(sourceType?: string, sourceId?: string): Transaction[] { const where = sourceType ? ' WHERE source_type=? AND source_id=?' : ''; return this.db.prepare(`SELECT id,user_id userId,season_id seasonId,quarter_id quarterId,scope,source_type sourceType,source_id sourceId,points,reason,author_id authorId,created_at createdAt,reverses_id reversesId FROM v2_points${where} ORDER BY rowid`).all(...(sourceType ? [sourceType, sourceId] : [])) as Transaction[]; }
    append(t: Omit<Transaction, 'id' | 'createdAt'>) {
        const row = { ...t, id: randomUUID(), createdAt: new Date().toISOString() };
        this.db.prepare('INSERT INTO v2_points VALUES (?,?,?,?,?,?,?,?,?,?,?,?)').run(row.id, row.userId, row.seasonId, row.quarterId, row.scope, row.sourceType, row.sourceId, row.points, row.reason, row.authorId, row.createdAt, row.reversesId);
        this.db.prepare('INSERT INTO v2_projection VALUES (?,?,?,?,?,?) ON CONFLICT(user_id,season_id,quarter_id,scope,source_type) DO UPDATE SET points=points+excluded.points').run(row.userId, row.seasonId, row.quarterId ?? '', row.scope, row.sourceType, row.points);
        this.audit('points', row.id, row.reversesId ? 'ESTORNO' : 'CREDITO', null, row, row.authorId);
        return row;
    }
    reverse(type: string, id: string, author: string, reason: string) { const rows = this.points(type, id); const reversed = new Set(rows.map(r => r.reversesId)); for (const r of rows)
        if (!r.reversesId && !reversed.has(r.id))
            this.append({ ...r, points: -r.points, reason, authorId: author, reversesId: r.id }); }
}
